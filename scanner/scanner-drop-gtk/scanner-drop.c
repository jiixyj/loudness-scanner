...
static GtkTreeModel *
create_result_list_model(GSList *files)
{
	GtkListStore *store;
	GtkTreeIter iter;
	...
	store = gtk_list_store_new(NUM_COLUMNS, G_TYPE_POINTER, G_TYPE_STRING,
	...
	while (files) {
		...
		if (fd->scanned) {
			...
			gtk_list_store_set(store, &iter, COLUMN_IS_TAGGED,
			    &fd->tagged, COLUMN_FILENAME, fln->fr->display,
			    ...
			    HIDDEN_FILENAME_COLLATE, fln->fr->collate_key, -1);
		}
...
static void
float_to_rg_display(GtkTreeViewColumn *tree_column, GtkCellRenderer *cell,
...
static void
is_tagged_to_icon_name(GtkTreeViewColumn *tree_column, GtkCellRenderer *cell,
    GtkTreeModel *tree_model, GtkTreeIter *iter, gpointer data)
{
	gpointer d;
	int tagged;
	...
	g_object_set(cell, "icon-name",
	    tagged ? (tagged == 1 ? GTK_STOCK_APPLY : GTK_STOCK_CANCEL) : "",
	    NULL);
}

static void
result_view_add_colums(GtkTreeView *treeview)
{
	...
	column = gtk_tree_view_column_new_with_attributes("Tagged", renderer,
	    NULL);
	gtk_tree_view_column_set_sort_column_id(column, COLUMN_IS_TAGGED);
	gtk_tree_view_append_column(treeview, column);
	gtk_tree_view_column_set_cell_data_func(column, renderer,
	    is_tagged_to_icon_name, (gpointer)COLUMN_IS_TAGGED, NULL);
	...
	renderer = gtk_cell_renderer_text_new();
	column = gtk_tree_view_column_new_with_attributes("Album Gain",
	    renderer, "text", COLUMN_ALBUM_GAIN, NULL);
	gtk_tree_view_column_set_sort_column_id(column, COLUMN_ALBUM_GAIN);
	gtk_tree_view_append_column(treeview, column);
	gtk_tree_view_column_set_cell_data_func(column, renderer,
	    float_to_rg_display, (gpointer)COLUMN_ALBUM_GAIN, NULL);

	renderer = gtk_cell_renderer_text_new();
	column = gtk_tree_view_column_new_with_attributes("Track Gain",
	    renderer, "text", COLUMN_TRACK_GAIN, NULL);
	gtk_tree_view_column_set_sort_column_id(column, COLUMN_TRACK_GAIN);
	gtk_tree_view_append_column(treeview, column);
	gtk_tree_view_column_set_cell_data_func(column, renderer,
	    float_to_rg_display, (gpointer)COLUMN_TRACK_GAIN, NULL);

	renderer = gtk_cell_renderer_text_new();
	column = gtk_tree_view_column_new_with_attributes("Album Peak",
	    renderer, "text", COLUMN_ALBUM_PEAK, NULL);
	gtk_tree_view_column_set_sort_column_id(column, COLUMN_ALBUM_PEAK);
	gtk_tree_view_append_column(treeview, column);

	renderer = gtk_cell_renderer_text_new();
	column = gtk_tree_view_column_new_with_attributes("Track Peak",
	    renderer, "text", COLUMN_TRACK_PEAK, NULL);
	gtk_tree_view_column_set_sort_column_id(column, COLUMN_TRACK_PEAK);
	gtk_tree_view_append_column(treeview, column);
}

static void
tag_files_and_update_model(GtkWidget *tag_button, struct work_data *wd)
{
	struct filename_list_node *fln;
	struct file_data *fd;
	int ret;
...
static void
destroy_work_data(struct work_data *wd)
{
...
static gboolean
show_result_list(struct work_data *wd)
{
	GtkTreeModel *model;
	GtkWidget *vbox;
	GtkWidget *sw;
	...
	gtk_tree_sortable_set_sort_func(GTK_TREE_SORTABLE(model),
	    COLUMN_IS_TAGGED, icon_sort, (gpointer)COLUMN_IS_TAGGED, NULL);
	gtk_tree_sortable_set_sort_func(GTK_TREE_SORTABLE(model),
	    COLUMN_FILENAME, filename_sort, (gpointer)HIDDEN_FILENAME_COLLATE,
	    NULL);
...
do_work(gpointer arg)
{
	...
	GSList *errors = NULL;
	wd->tree = filetree_init(wd->roots, wd->length, TRUE, FALSE, FALSE,
	    &errors);
...
static void
handle_popup(GtkWidget *widget, GdkEventButton *event)
{
...
static gboolean
handle_button_press(GtkWidget *widget, GdkEventButton *event, gpointer *unused)
{
	(void)unused;
...
static gboolean
handle_expose(GtkWidget *widget, GdkEventExpose *event, RsvgHandle *rh)
{
	...
	for (;;) {
		...
		if (total_frames > 0) {
			...
			if (!rotation_active && elapsed_frames) {
				g_timeout_add(40, (GSourceFunc)rotate_logo,
				    widget);
				rotation_active = TRUE;
			}
			frac = gtk_progress_bar_get_fraction(
			    GTK_PROGRESS_BAR(progress_bar));
			new_frac = CLAMP((double)elapsed_frames /
				(double)total_frames,
			    0.0, 1.0);
...
static GtkActionEntry action_entries[] = { { "FileMenuAction", NULL, "_File",
					       NULL, NULL, NULL },

	{ "QuitAction", GTK_STOCK_QUIT, "_Quit", "<control>Q", "Quit",
	    G_CALLBACK(exit_program) } };

static char const *ui = "<ui>"
...
main(int argc, char *argv[])
{
	...
	g_object_set(window, "user-data",
	    gtk_menu_item_get_submenu(GTK_MENU_ITEM(
		gtk_ui_manager_get_widget(menu_manager, "/MainMenu/FileMenu"))),
	    NULL);
