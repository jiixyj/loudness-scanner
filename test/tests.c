...
double
test_global_loudness(char const *filename)
{
	SF_INFO file_info;
	SNDFILE *file;
	...
	st = ebur128_init((size_t)file_info.channels,
	    (size_t)file_info.samplerate, EBUR128_MODE_I);
...
double
test_loudness_range(char const *filename)
{
	SF_INFO file_info;
	SNDFILE *file;
	sf_count_t nr_frames_read;
	...
	st = ebur128_init((size_t)file_info.channels,
	    (size_t)file_info.samplerate, EBUR128_MODE_LRA);
	if (file_info.channels == 5) {
		ebur128_set_channel(st, 0, EBUR128_LEFT);
		ebur128_set_channel(st, 1, EBUR128_RIGHT);
		ebur128_set_channel(st, 2, EBUR128_CENTER);
...
double
test_true_peak(char const *filename)
{
	SF_INFO file_info;
	SNDFILE *file;
	sf_count_t nr_frames_read;
...
double lrae[] = { 1.0001105488329134e+01, 4.9993734051522178e+00,
	1.9995064067783115e+01, 1.4999273937723455e+01, 4.9747585878473721e+00,
...
#define TEST_GLOBAL_LOUDNESS(filename, i)                                 \
	...
	if (result == result) {                                           \
		printf("%s, %s - %s: %1.16e\n",                           \
		    (result <= gr[i] + 0.1 && result >= gr[i] - 0.1) ?    \
			      "PASSED" :                                        \
			      "FAILED",                                         \
